...
// Mock moxen-client with a factory so we can swap the impl per test
let mockSm: {
...
describe('GET /api/goals', () => {
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moxen-web-goals-'));
    mockSm = {
...
describe('GET /api/goals/:id/tasks', () => {
  ...
  it('returns tasks from JSON files sorted by created_at desc', async () => {
    const tasksDir = path.join(tmpDir, '.moxen', 'tasks', 'goal-1');
