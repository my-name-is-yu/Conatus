...
# 7 curated goals aimed at improving Moxen itself.
# Keep descriptions concrete and verification-oriented so Moxen can drive changes.
GOALS=(
  "Fix 'moxen suggest' so it returns actionable improvement goals for this repo. Ensure it uses repo context (--path .) and doesn't prematurely return empty. Align behavior with docs/design where applicable. Add/adjust tests."
  ...
  "Stability: make 'moxen run' resilient to adapter failures/timeouts (clear reporting, non-zero exits, no silent success). Add tests to cover adapter error propagation."
...
say "# Moxen overnight loop (${DATE_TAG})" > "$REPORT"
say "" >> "$REPORT"
...
for i in $(seq 1 7); do
  ...
  # 2) Run Moxen for that goal (bounded iterations)
  set +e
  ...
  say "## Moxen run" >> "$REPORT"
  say "- command: moxen run --goal ${GOAL_ID} --yes --max-iterations 12" >> "$REPORT"
  ...
  # 3) Verify repo health after Moxen-run-driven changes
  set +e
