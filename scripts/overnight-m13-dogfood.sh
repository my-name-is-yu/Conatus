...
GOAL1_OUTPUT=$($MOXEN goal add --title "zero-tsc-errors" --dim "tsc_error_count:min:0" 2>&1)
...
if [ -z "$GOAL1_ID" ]; then
  ...
  $MOXEN run --goal "$GOAL1_ID" --yes --max-iterations 3 2>&1 | tee "$LOG_DIR/phase1.log"
...
GOAL2_OUTPUT=$($MOXEN goal add --title "improve-test-coverage" --dim "test_coverage:min:80" --dim "test_count:min:4000" 2>&1)
...
if [ -z "$GOAL2_ID" ]; then
  ...
  $MOXEN run --goal "$GOAL2_ID" --yes --max-iterations 2 2>&1 | tee "$LOG_DIR/phase2.log"
...
$MOXEN plugin list 2>&1 | tee -a "$LOG_DIR/summary.log"
