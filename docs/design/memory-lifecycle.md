...
> Motivaは年単位で動き続ける。データは無限に蓄積するが、LLMのコンテキストウィンドウは有限だ。
...
Motivaが長期運用されると、以下のデータが継続的に蓄積される。
...
「古いものから消す」は記憶管理の最も単純な戦略だ。しかしMotiva においてはこれが機能しない。
...
| 層 | 保持内容 | 形式 |
|----|---------|------|
| Working | 含めない（レポートはMotiva→ユーザーの出力であり、ループの入力ではない） |  |
...
Motivaには「何が重要か」を判断する機構が既にある。DriveScorer（不満・締切・機会）とSatisficingJudge（十分かどうか）だ。汎用的なLRUではなく、この既存の動機システムを記憶管理に活用する。

### 7.1 DriveScorer連携

**不満駆動: 高不満の次元に関連する記憶は圧縮を遅延する。**

ギャップが大きい次元は、Motivaが集中的に取り組んでいる領域だ。この次元に関連するShort-termデータは、通常の保持期間を超えても圧縮せずに保持する。
...
SatisficingJudge（`satisficing.md`）が「この次元は十分」と判定した場合、その次元のShort-termデータは保持期間を待たずにLong-term圧縮の対象になる。Motivaが注意を向ける必要がない次元の生データを保持し続ける意味はない。
