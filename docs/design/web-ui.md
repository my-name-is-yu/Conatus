...
Moxen is the "control room for agents." Like a cockpit, a large amount of information is arranged in organized positions, and attention is drawn only when something is anomalous.
...
| SaaS blue + light gray | Dark base + amber accent (warm color to express "moxention") |
...
├── lib/  (moxen-client.ts, store.ts, sse.ts)
├── styles/tokens.css
...
Route Handlers directly import moxen-core modules within the Node.js process (same process, no RPC needed).
...
Provider settings (`~/.moxen/provider.json`), plugin management (enable/disable), system health (CoreLoop state, EventServer connection count)
...
- [ ] TUI and Web UI can operate concurrently against the same Moxen process
