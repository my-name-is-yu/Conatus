...
Tavori has two separate paths for making goals concrete. `negotiate()` in GoalNegotiator runs a 6-step flow: LLM dimension decomposition, baseline estimation, feasibility evaluation (qualitative or quantitative), capability check, and response generation. It produces a single goal with validated, feasibility-checked dimensions. `decomposeGoal()` in GoalTreeManager runs a different flow: LLM specificity scoring, recursive sub-goal generation, coverage validation, and cycle detection. It produces a tree of goals, each with dimensions assigned by the decomposition LLM.
