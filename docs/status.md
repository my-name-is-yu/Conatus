...
- `src/cli-runner.ts` extended — `moxen suggest`, `moxen improve [path]` commands
- Status: complete
...
- `src/runtime/plugin-loader.ts` — dynamic plugin loading from `~/.moxen/plugins/`, manifest validation, auto-registration into AdapterRegistry/DataSourceRegistry/NotifierRegistry
...
- `src/runtime/event-server.ts` extended — real-time file watcher for `~/.moxen/events/` (fs.watch)
