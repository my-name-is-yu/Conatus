...
> **実装状況（2026-03-15時点）**: Stage 1-12完了 — 1919テスト通過、40テストファイル。
> 実装済みモジュール: （Stage 12追加分）EmbeddingClient, VectorIndex, KnowledgeGraph, GoalDependencyGraph（Layer 12）を追加。
> Zodスキーマ: 25ファイル（embedding.ts, dependency.ts, data-source.ts, capability.ts を追加）
> 次ステップ: Stage 13（能力自律調達と外部世界接続）。詳細は docs/roadmap.md。
