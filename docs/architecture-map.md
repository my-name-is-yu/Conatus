# Moxen -- Architecture Map

---

## 1. In a Nutshell

Moxen is a **task discovery engine**. It takes on the user's long-term goals ("I want to double revenue," "I want to live happily with my dog"), observes the real world, and keeps discovering "what should be done next" from the gap with the goal. Moxen itself executes nothing. It delegates discovered tasks to AI agents, verifies the results, and runs the loop again. Until the goal is achieved — days or years, however long it takes.
...
```
 ...
 │   During execution: Moxen does not intervene                     │
...
|---|---|---|---|---|
| `vision.md` | Vision. Definition of the world Moxen enables | -- | All design documents | Designed |
...
| `character.md` | Moxen persona definition (4 behavioral axes, LLM prompt spec) | -- | goal-negotiation.md, reporting.md, stall-detection.md | Designed |
...
| `curiosity.md` | Curiosity (meta-moxention, new goal proposals) | satisficing.md, stall-detection.md | goal-negotiation.md (new goals) | Designed |
...
| `execution-boundary.md` | Execution boundary (what Moxen does and what it delegates) | mechanism.md | task-lifecycle.md | Designed |
...
| `drive-system.md` | File-queue method (`~/.moxen/events/`) for event reception confirmed as MVP design | drive-system.md |
...
**Curiosity (curiosity)**
Goal-level meta-moxention. Triggered by empty task queue / unexpected observation / repeated failures / periodic exploration. Two functions: new goal proposal and existing goal redefinition. Always lower priority than user goals. Unaccepted proposals automatically expire after 12 hours.
...
"Moxen thinks. Agents act." Moxen directly performs only LLM calls (for thinking) and reading/writing state files. Everything else (code implementation, data collection, notification delivery, etc.) is delegated to agents.
