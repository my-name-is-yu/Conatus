...
Moxen — AI agent orchestrator that gives existing agents the drive to persist. Moxen sits above agents and drives them: selecting goals, spawning agent sessions, observing results, and judging completion. Moxen doesn't think — it makes agents think.
...
- Moxen calls LLMs (for goal decomposition, observation) — it is the caller, not the callee
- Execution boundary: Moxen always delegates. Direct actions are LLM calls (for thinking) and state read/write only
...
- State persistence: file-based JSON (~/.moxen/)
- Test: vitest
...
- Layer 15: PluginLoader, NotifierRegistry, INotifier (plugin architecture — dynamic load from `~/.moxen/plugins/`, notifier routing)

## Design Documents

- `docs/vision.md` — why Moxen exists
...
- **Dogfooding推奨モデル**: gpt-5.3-codex（gpt-4o-miniより観測精度・収束速度が大幅に優れる。`~/.moxen/provider.json`で設定）
