 ...
 *   1. MOXEN_LLM_PROVIDER environment variable
 *   2. ~/.moxen/provider.json llm_provider field
 *   3. Default: OpenAI
 *
 * Providers:
...
export async function buildAdapterRegistry(
  ...
  const envBaseUrl = process.env["MOXEN_A2A_BASE_URL"];
  if (envBaseUrl && !config.a2a?.agents) {
    registry.register(new A2AAdapter({
      baseUrl: envBaseUrl,
      authToken: process.env["MOXEN_A2A_AUTH_TOKEN"],
    }));
  }
