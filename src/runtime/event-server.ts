...
import { MoxenEventSchema } from "../types/drive.js";
...
export interface EventServerConfig {
  ...
  eventsDir?: string; // default: ~/.moxen/events/
}
...
export class EventServer {
  ...
  constructor(driveSystem: DriveSystem, config?: EventServerConfig, logger?: Logger) {
    ...
    // Default events directory: ~/.moxen/events/
   ...
   * When a file appears:
   *   1. Read and parse it
   *   2. Validate via MoxenEventSchema
  ...
  private handleRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
    ...
    req.on("end", () => {
      try {
        ...
        const event = MoxenEventSchema.parse(data);
