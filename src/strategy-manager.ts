...
import { StrategySchema, PortfolioSchema, WaitStrategySchema } from "./types/strategy.js";
...
export class StrategyManager {
   ...
   */
  terminateStrategy(goalId: string, strategyId: string, reason: string): Strategy {
    const portfolio = this.loadOrCreatePortfolio(goalId);
    const strategy = portfolio.strategies.find((s) => s.id === strategyId);
    if (!strategy) {
      throw new Error(
    ...
    // Remaining active strategies (excluding this one)
    const remaining = portfolio.strategies.filter(
      (s) => s.id !== strategyId && (s.state === "active" || s.state === "evaluating")
    );

    // Redistribute freed allocation proportionally
    let updatedStrategies = portfolio.strategies.map((s) =>
