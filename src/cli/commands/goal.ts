...
export async function cmdGoalAdd(
  ...
  try {
    ...
    console.log(`\nTo run the loop: moxen run --goal ${goal.id}`);
    return 0;
  } catch (err) {
...
export async function cmdGoalList(
  ...
  if (goalsDirEntries.length === 0) {
    console.log("No goals registered. Use `moxen goal add` to create one.");
  } else {
  ...
  if (opts.archived && archivedIds.length > 0) {
    ...
    console.log(`\nArchived goals: ${archivedIds.length} (use \`moxen goal list --archived\` to show)`);
  }
...
export async function cmdStatus(stateManager: StateManager, goalId: string): Promise<number> {
  ...
  if (execReports.length > 0) {
    ...
    console.log(`\n_No execution reports yet. Run \`moxen run --goal ${goalId}\` to start._`);
  }
...
export async function cmdGoalReset(stateManager: StateManager, goalId: string): Promise<number> {
  ...
  console.log(`\nRun \`moxen run --goal ${goalId}\` to restart the loop.`);
...
export async function cmdCleanup(stateManager: StateManager): Promise<number> {
  ...
  if (staleReports.length > 0) {
    ...
    console.log("(These can be removed manually from ~/.moxen/reports/)");
  }
